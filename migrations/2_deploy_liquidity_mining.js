...
module.exports = function(deployer, network, accounts) {
  deployer.then(async () => {
    ...
    const budget = BN(duration).mul(BN(rewardPerBlock)).toString();
    console.log("Budget: ", budget);

    // console.log("Deploying TRewardToken...");
    // await wait(await deployer.deploy(TRewardToken, budget));
    // const rewardToken = await TRewardToken.deployed();
    ...
    await wait(await deployer.deploy(Reservoir));
    ...
    await wait(await deployer.deploy(LiquidityMining,
      rewardTokenAddress,
      reservoir.address,
      rewardPerBlock,
      startBlock,
    ...
    await wait(await reservoir.setApprove(
      rewardTokenAddress,
